const FavouriteCities = ({
  ...
  return (
    <div className=" p-4 forecast rounded-lg shadow-md w-80">
      ...
      {favoriteCities.length > 0 ? (
        <ul>
          {favoriteCities.map((city, index) => (
            <li
              key={index}
              className="cursor-pointer p-2 border-b border-gray-300 flex justify-between hover:text-black hover:bg-gray-200"
              onClick={() => fetchWeather(city)}
            >
              {city.name}
              <button
                onClick={() => removeFavorite(city)}
                className="bg-red-500 px-2 py-1 cursor-pointer text-white "
              >
                ❌ Премахни
              </button>
            </li>
